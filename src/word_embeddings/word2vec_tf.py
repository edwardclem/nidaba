...
filename = '../../data/processed/ur3_etcsri.txt.zip'
...
with tf.Session(graph=graph) as session:
    ...
    embeddings_txt = "../../results/embed/70k.txt"

    with open(embeddings_txt, 'w') as f:
        f.write("{} {}\n".format(vocabulary_size, embedding_size))
        outstrings = []
        for i in range(vocabulary_size):
            word = reverse_dictionary[i]
            emb = final_embeddings[i,:]
            outstrings.append("{} {}".format(word, " ".join([str(e) for e in emb])))
        f.write("\n".join(outstrings))

    print("saved to {}".format(embeddings_txt))
